...
class AdminUploadPage extends State<AdminUpload> {
  ...
  final String uploadUrl =
      'https://zoological-wafer.000webhostapp.com/EWishes/category/upload_category_main.php';
  ...
  final String categoryUploadUrl =
      'https://zoological-wafer.000webhostapp.com/EWishes/category/upload_category_sub_image_insert.php';
  ...
  Future<String?> categoryUploadImage(filepath, url) async {
    File file = File(filepath.path);
    var request = http.MultipartRequest('POST', Uri.parse(url));
    request.fields['category_name'] = _myCategory.toString();
    request.files
        .add(await http.MultipartFile.fromPath('profile_pic', file.path));
    var res = await request.send();
    print(file.path);
